...
- [[#backends][Backends]]
  ...
  - [[#language-server-protocol][Language Server Protocol]]
    ...
    - [[#ty-language-server][ty language server]]
...
- Support for the following backends:
  ...
  - [[https://github.com/emacs-lsp/lsp-mode][Language Server Protocol]] (3 implementations),
    - python-lsp-server
    - Microsoft pyright language server
    - ty language server
- Auto-completion
- Code Navigation
- Documentation Lookup using [[https://github.com/proofit404/anaconda-mode][anaconda-mode]]
...
| 'ty      | [[https://docs.astral.sh/ty/][ty language server]]                  |
...
To use the ty language server, set the =python-lsp-server= layer variable as
follows:

#+BEGIN_SRC elisp
  (python :variables python-backend 'lsp python-lsp-server 'ty)
#+END_SRC
...
*** ty language server
[[https://docs.astral.sh/ty/][Ty]] is an extremely fast Python type checker and language server written in Rust
by Astral (the creators of Ruff and uv). See the [[https://docs.astral.sh/ty/installation/][official installation guide]] for
all methods.

#+BEGIN_SRC sh
  # project (recommended, also [[#manage-environments-and-packages-with-uv][enable uv support]])
  uv add --dev ty
...
Note that the =pyright= and =ty= language servers do not support formatting,
but they can be used alongside with =ruff=, which does support it.
